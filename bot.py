...
from helpers.handle_creds import (
    load_correct_creds, test_api_key, load_telegram_creds
)
...
def wait_for_price():
    ...
    send_message(f'Working...Session profit:{session_profit:.2f}% Est:${(QUANTITY * session_profit)/100:.2f}')
        ...
        if threshold_check > CHANGE_IN_PRICE:
            ...
            if datetime.now() >= volatility_cooloff[coin] + timedelta(minutes=TIME_DIFFERENCE):
                ...
                if len(coins_bought) + len(volatile_coins) < MAX_COINS or MAX_COINS == 0:
                    ...
                    send_message(f'{coin} has gained {volatile_coins[coin]}% within the last {TIME_DIFFERENCE} minutes, calculating volume in {PAIR_WITH}')

                else:
                    send_message(f'{txcolors.WARNING}{coin} has gained {round(threshold_check, 3)}% within the last {TIME_DIFFERENCE} minutes, but you are holding max number of coins{txcolors.DEFAULT}')
    ...
    for excoin in externals:
        if excoin not in volatile_coins and excoin not in coins_bought and (len(coins_bought) + exnumber) < MAX_COINS:
            ...
            exnumber +=1
            send_message(f'External signal received on {excoin}, calculating volume in {PAIR_WITH}')
...
def pause_bot():
    ...
    while os.path.isfile("signals/paused.exc"):
        ...
        if bot_paused == False:
            send_message(f'{txcolors.WARNING}Pausing buying due to change in market conditions, stop loss and take profit will continue to work...{txcolors.DEFAULT}')
            bot_paused = True
        ...
        # pausing here
        if hsp_head == 1: send_message(f'Paused...Session profit:{session_profit:.2f}% Est:${(QUANTITY * session_profit)/100:.2f}')
        ...
        if  bot_paused == True:
            send_message(f'{txcolors.WARNING}Resuming buying due to change in market conditions, total sleep time: {time_elapsed}{txcolors.DEFAULT}')
            bot_paused = False
...
def buy():
    ...
    for coin in volume:
        ...
        if coin not in coins_bought:
            send_message(f"{txcolors.BUY}Preparing to buy {volume[coin]} {coin}{txcolors.DEFAULT}")
            ...
            # run the else block if the position has been placed and return order info
                ...
                while orders[coin] == []:
                    send_message('Binance is being slow in returning the order, calling the API again...')

                    orders[coin] = client.get_all_orders(symbol=coin, limit=1)
                    time.sleep(1)

                else:
                    send_message('Order returned, saving order to file')
            ...
            send_message(f'Signal detected, but there is already an active trade on {coin}')
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        if LastPrice < SL or LastPrice > TP and not USE_TRAILING_STOP_LOSS:
            send_message(f"{txcolors.SELL_PROFIT if PriceChange >= 0. else txcolors.SELL_LOSS}TP or SL reached, selling {coins_bought[coin]['volume']} {coin} - {BuyPrice} - {LastPrice} : {PriceChange-(TRADING_FEE*2):.2f}% Est:${(QUANTITY*(PriceChange-(TRADING_FEE*2)))/100:.2f}{txcolors.DEFAULT}")
        ...
        if hsp_head == 1:
            if len(coins_bought) > 0:
                send_message(f'TP or SL not yet reached, not selling {coin} for now {BuyPrice} - {LastPrice} : {txcolors.SELL_PROFIT if PriceChange >= 0. else txcolors.SELL_LOSS}{PriceChange-(TRADING_FEE*2):.2f}% Est:${(QUANTITY*(PriceChange-(TRADING_FEE*2)))/100:.2f}{txcolors.DEFAULT}')

    if hsp_head == 1 and len(coins_bought) == 0: send_message(f'Not holding any coins')
...
def update_portfolio(orders, last_price, volume):
    ...
    for coin in orders:
        ...
        send_message(f'Order with id {orders[coin][0]["orderId"]} placed and saved to file')
...
def send_message(textMessage):
    if TELEGRAM:
        print(textMessage)
        time.sleep(3)
        dp.bot.send_message(chat_id=chatId, text=textMessage)
        # Flood control
    else:
        print(textMessage)
...
if __name__ == '__main__':
    ...
    if TELEGRAM:
        ...
        # Load creds for correct environment
        api_key, chatId = load_telegram_creds(parsed_creds)
    ...
    if not TEST_MODE:
        if not args.notimeout: # if notimeout skip this (fast for dev tests)
            send_message('WARNING: You are using the Mainnet and live funds. Waiting 30 seconds as a security measure')
            time.sleep(30)
    ...
    for filename in signals:
        for line in open(filename):
            try:
                ...
                if DEBUG: send_message(f'{txcolors.WARNING}Could not remove external signalling file {filename}{txcolors.DEFAULT}')
    ...
    try:
        if len(SIGNALLING_MODULES) > 0:
            for module in SIGNALLING_MODULES:
                send_message(f'Starting {module}')
            ...
            send_message(f'No modules to load {SIGNALLING_MODULES}')
    except Exception as e:
        print(e)
